...
const createDemoWorkspace = async user => {
  try {
    ...
    //2.Find cluster and add crendentials ( cluster is open but we still need to add empty creds so user wont be prompted)
    ...
    const dashboard = await findOrCreateDashboard(
      ...
      user.id,
      defaultDashboardName,
      null, // file name , we dont want to assosiate any filename to default dashboard
      'Owner',
    );
